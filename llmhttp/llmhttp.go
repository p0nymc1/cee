...
func Extractor(cfg Config, fields []string) llminjector.Extractor {
	...
	return func(rawText string) (map[string]any, error) {
		reqBody, err := json.Marshal(chatRequest{
			Model:       cfg.Model,
			Temperature: 0, // deterministic-as-possible extraction, not creativity
		...
		if err != nil {
			return nil, fmt.Errorf("llmhttp: reading response: %w", err)
		}
		if resp.StatusCode != http.StatusOK {
			return nil, fmt.Errorf("llmhttp: endpoint returned %d: %s", resp.StatusCode, strings.TrimSpace(string(body)))
		}

		var parsed chatResponse
		if err := json.Unmarshal(body, &parsed); err != nil {
			return nil, fmt.Errorf("llmhttp: cannot decode response envelope: %w", err)
		}
		if len(parsed.Choices) == 0 {
			return nil, fmt.Errorf("llmhttp: response contained no choices")
		}
