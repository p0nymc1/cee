...
> 中文版：[TECHNICAL_SPECIFICATION.md](TECHNICAL_SPECIFICATION.md)
...
- **Real backends**: **mostly delivered; only the built-in sandbox is still an in-process simulation.**
  ...
  Distributed orchestration (Temporal-style semantics) is **explicitly out of scope** — see the boundaries in the
  whitepaper's chapter 6.
...
- **Only output is measured, not error**: `scorecard` measures deterministic steps and calls eliminated — all
  flattering numbers. There is not a single **diagnostic** metric: intent miss rate, probe refusal rate, escalation
  rate. Measuring only the flattering numbers is a form of bias, and this should be treated as a known defect rather
  than a "later" item.
